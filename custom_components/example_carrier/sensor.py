"""Sensor platform for the Example Carrier parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: ExampleCarrierConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Example Carrier sensor entities from a config entry."""
    ...
    for parcel in coordinator.data or []:
        entities.append(
            ExampleCarrierParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(ExampleCarrierNextDeliverySensor(coordinator, entry))
    entities.append(ExampleCarrierDeliveredParcelsSensor(coordinator, entry))
    entities.append(ExampleCarrierLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class ExampleCarrierIncomingParcelsSensor(
    CoordinatorEntity[ExampleCarrierCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                ExampleCarrierParcelSensor(self.coordinator, self._entry, barcode)
...
class ExampleCarrierParcelSensor(CoordinatorEntity[ExampleCarrierCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Example Carrier parcel."""
    ...
    def __init__(
        self, coordinator: ExampleCarrierCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class ExampleCarrierNextDeliverySensor(
    CoordinatorEntity[ExampleCarrierCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: ExampleCarrierCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class ExampleCarrierDeliveredParcelsSensor(
    CoordinatorEntity[ExampleCarrierCoordinator], SensorEntity
):
    """Recently delivered tracked Example Carrier parcels."""
...
class ExampleCarrierLastUpdateSensor(
    CoordinatorEntity[ExampleCarrierCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Example Carrier was last polled successfully."""
