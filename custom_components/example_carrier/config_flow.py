...
class ExampleCarrierConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(
        ...
        """Create the Example Carrier hub — single instance, no input needed.
        ...
        the ``example_carrier.track_parcel`` service or a dashboard button.
        ``single_config_entry`` in the manifest enforces one hub.

        TODO(carrier): if the carrier keys tracking on code *plus* postcode,
        ask for the postcode here and store it per parcel alongside the code —
        do not add a second config entry for it.
        """
