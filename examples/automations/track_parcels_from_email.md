# Automatic parcel tracking from e-mail (IMAP → Example Carrier)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Example Carrier tracking code, and register it with `example_carrier.track_parcel` — fully automatic, no extra custom component required.

Example Carrier is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `example_carrier.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ example_carrier.track_parcel
                                     │
...
- This integration, with the `example_carrier.track_parcel` action available (field `tracking_code`).
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@example_carriershipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `example_carrier.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
