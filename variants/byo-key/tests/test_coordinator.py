"""Tests for the Example Carrier coordinator: fetching, caching and events.
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = ExampleCarrierApiError("HTTP 500")
...
async def test_update_raises_auth_failed_when_key_is_rejected(hass):
    ...
    entry.add_to_hass(hass)
    client = AsyncMock()
    client.async_get_parcel.side_effect = ExampleCarrierAuthError("HTTP 401")
    coordinator = ExampleCarrierCoordinator(hass, client, entry)
...
async def test_auth_failure_on_one_parcel_still_raises_auth_failed(hass):
    ...
    client.async_get_parcel.side_effect = lambda code: (
        active_sample(code)
        if code == ACTIVE_CODE
        else (_ for _ in ()).throw(ExampleCarrierAuthError("HTTP 401"))
    )
    coordinator = ExampleCarrierCoordinator(hass, client, entry)
