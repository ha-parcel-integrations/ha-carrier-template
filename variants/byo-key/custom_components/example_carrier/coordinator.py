...
class ExampleCarrierCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for result in results:
            if isinstance(result, ExampleCarrierAuthError):
                raise ConfigEntryAuthFailed("Example Carrier API key rejected") from result
        ...
        for code, result in zip(codes_to_fetch, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (ExampleCarrierApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                if (
                    isinstance(result, ExampleCarrierApiError)
                    and result.status_code == 429
                ):
                    saw_429 = True
                    if result.retry_after is not None:
                        retry_afters.append(result.retry_after)
                _LOGGER.warning("Example Carrier fetch failed for %s: %s", code, result)
        ...
        if codes_to_fetch and errors == len(codes_to_fetch) and not raws_by_code:
            raise UpdateFailed("Example Carrier unreachable for all tracked parcels")
