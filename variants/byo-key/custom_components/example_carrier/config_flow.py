...
# TODO(carrier): switch to two fields (client_id/client_secret) if the carrier
# issues an OAuth client-credentials pair instead of a single key — see the
# TODO on ExampleCarrierApiClient in api.py.
_KEY_SCHEMA = vol.Schema({vol.Required(CONF_API_KEY): str})
...
class ExampleCarrierConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def _validate(self, api_key: str) -> None:
        ...
        """
        client = ExampleCarrierApiClient(api_key, async_get_clientsession(self.hass))
    ...
    async def async_step_user(
        ...
        TODO(carrier): if the carrier keys tracking on code *plus* postcode,
        ask for the postcode per parcel in the options flow — do not add a
        ...
        if user_input is not None:
            try:
                await self._validate(user_input[CONF_API_KEY])
